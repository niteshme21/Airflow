...
from sqlalchemy import Column, String, DateTime, Integer, Text, Boolean, Index, ForeignKey, Enum
...
class DependencyExecution(Base):
    ...
    dependency_id = Column(Integer, ForeignKey('cross_dag_dependency.id'), nullable=False)
    ...
    # Status tracking
    status = Column(Enum(DependencyStatus), default=DependencyStatus.PENDING, index=True)
...
class DependencyAuditLog(Base):
    ...
    # Operation details
    operation_type = Column(String(50), nullable=False)  # CREATE, UPDATE, DELETE, CHECK, APPROVE
...
class EventTriggerRule(Base):
    ...
    __table_args__ = (
        Index('idx_dag_trigger', 'dag_id', 'trigger_type'),
    )
...
class DependencyMetrics(Base):
    ...
    # Metric details
    metric_name = Column(String(250), nullable=False)  # dependency_check_duration, circular_dependencies_detected, etc.

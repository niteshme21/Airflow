...
class DependencyManagementHook(BaseHook):
        ...
        try:
            ...
            dependency = CrossDAGDependency(
                ...
                dependency_type=(
                    ...
                    else DependencyType.TASK_TO_DAG
                ),
            ...
            logger.info(
                f"Created dependency {dependency.id}: {source_dag_id} -> {dependent_dag_id}"
            )
            return dependency
    ...
    def get_all_dependencies(self) -> List:
        ...
        try:
            dependencies = (
                session.query(CrossDAGDependency)
                .filter(CrossDAGDependency.is_active == True)
                .all()
            )
            return dependencies
        finally:
    ...
    def detect_circular_dependency(
        self, source_dag_id: str, dependent_dag_id: str
    ) -> bool:
        """
            ...
            for node in graph:
                if node not in visited:
                    if has_cycle(node, visited, set()):
                        logger.warning(
                            f"Circular dependency detected: {source_dag_id} -> {dependent_dag_id}"
                        )
                        return True
    ...
    def get_unresolved_circular_dependencies(self) -> List:
        ...
        try:
            unresolved = (
                session.query(CircularDependencyDetection)
                .filter(CircularDependencyDetection.is_resolved == False)
                .all()
            )
            return unresolved
        finally:
    ...
    def get_stuck_dependencies(self, timeout_minutes: int = 60) -> List:
        ...
        try:
            ...
            stuck = (
                session.query(DependencyExecution)
                .filter(
                    and_(
    ...
    def _validate_dags_exist(
        self, source_dag_id: str, dependent_dag_id: str, session: Session
    ):
        """Validate that both DAGs exist in Airflow"""
        from airflow.models import DagModel

        source_dag = (
            session.query(DagModel).filter(DagModel.dag_id == source_dag_id).first()
        )
        if not source_dag:
            raise ValueError(f"Source DAG {source_dag_id} does not exist")

        dependent_dag = (
            session.query(DagModel).filter(DagModel.dag_id == dependent_dag_id).first()
        )

...
from .models.dag_dependency import (
    CrossDAGDependency, DependencyExecution, CircularDependencyDetection,
    DependencyAuditLog, DependencyStatus, DependencyType
)
...
class CrossDAGDependencySensor(BaseSensorOperator):
    ...
    def _check_source_status(self, execution_date: datetime, context: Dict) -> bool:
        ...
        try:
            ...
            source_run = session.query(DagRun).filter(
                and_(
            ...
            if self.source_task_id:
                ...
                is_success = task_instance and task_instance.state == TaskInstanceState.SUCCESS
            else:
...
class HealthCheckOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> Dict[str, Any]:
        ...
        try:
            ...
            health_report['checks']['database'] = self.hook.check_database_connectivity()
